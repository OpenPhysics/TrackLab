...
function axesIcon(): Node {
  const xArrow = new ArrowNode( 0, ICON_SIZE * 0.7, ICON_SIZE * 0.85, ICON_SIZE * 0.7, {
    fill: TrackLabColors.axisXColorProperty, stroke: null, headWidth: 5, headHeight: 5, tailWidth: 1.5,
  } );
  const yArrow = new ArrowNode( 0, ICON_SIZE * 0.7, 0, ICON_SIZE * 0.05, {
    fill: TrackLabColors.axisYColorProperty, stroke: null, headWidth: 5, headHeight: 5, tailWidth: 1.5,
  } );
  ...
  return new Node( {
    children: [
      new Line( cx - half, cy, cx + half, cy, {
        stroke: calColor, lineWidth: 1.5, lineDash: [ 3, 2 ],
      } ),
